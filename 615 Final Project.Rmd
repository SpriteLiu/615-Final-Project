...
nhr <- unique(nhr)
result <- table(nhr$route_id,nhr$month)
result <- as.data.frame(result)
names(result) <- c("Line","Month","Times")
result$Times <- round(result$Times/7)
result$Month <- factor(result$Month,levels = c("October","November","December","January","February","March","April","May","June","July","August","September"))
ggplot(data = result, mapping = aes(x = Month, y = Times, group = Line)) +   
geom_line(aes(linetype=Line),colour="blue")+ 
